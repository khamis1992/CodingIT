...
export class GitHubErrorBoundary extends Component<Props, State> {
  ...
  render() {
    if (this.state.hasError) {
      ...
      return (
        <div className="flex flex-col items-center justify-center p-8 text-center space-y-4 bg-codinit-elements-background-depth-1 border border-codinit-elements-borderColor rounded-lg">
          <div className="w-12 h-12 rounded-full bg-red-50 dark:bg-red-900/20 flex items-center justify-center">
            <AlertTriangle className="w-6 h-6 text-red-500" />
          </div>

          <div>
            <h3 className="text-lg font-medium text-codinit-elements-textPrimary mb-2">GitHub Integration Error</h3>
            ...
            {this.state.error && (
              <details className="text-xs text-codinit-elements-textTertiary mb-4">
                <summary className="cursor-pointer hover:text-codinit-elements-textSecondary">
                  Show error details
                </summary>
                <pre className="mt-2 p-2 bg-codinit-elements-background-depth-2 rounded text-left overflow-auto">
